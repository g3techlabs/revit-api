...
func (c *EventController) GetEventSubscribers(ctx *fiber.Ctx) error {
	query := new(input.GetEventSubscribersInput)
	...
	response, err := c.eventService.GetEventSubscribers(userId, eventId, query)
	if err != nil {
		return err
	}
...
func (c *EventController) CancelEvent(ctx *fiber.Ctx) error {
	eventParam := ctx.Params("eventId")
	eventIdUint64, err := strconv.ParseUint(eventParam, 10, 64)
	if err != nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(fiber.Map{
	...
	if err := c.eventService.CancelEvent(userId, eventId); err != nil {
		return err
	}
